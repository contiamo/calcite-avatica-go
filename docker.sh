...
check_import_paths(){
    ...
    if ! grep -Fq "module github.com/contiamo/calcite-avatica-go/$TAG_MAJOR_VERSION" go.mod; then
    ...
    for i in $(git ls-files); do
        ...
        lines=$(grep -F -s '"github.com/contiamo/calcite-avatica-go' $i) || true

        if ! [[ -z "$lines" ]]; then
            while read -r line; do
                if ! grep -q "github.com/contiamo/calcite-avatica-go/$TAG_MAJOR_VERSION" <<< "$line" ; then
                    BAD_IMPORT_PATHS=true
                    echo "Import for github.com/contiamo/calcite-avatica-go in $i does not have the correct version ($TAG_MAJOR_VERSION) in its path"
                fi
            done <<< "$lines"
        fi
    done
...
https://github.com/contiamo/calcite-avatica-go/blob/$COMMIT/site/_docs/go_history.md
